...
function sendSms(smsOptions, cb) {
	...
	pduObj.params  = {
		'source_addr_ton': smsOptions.hasOwnProperty('srcAddrTon') ? smsOptions.srcAddrTon :	1, // Default to international format
	...
	if (smsOptions.hasOwnProperty('encoding')){
		pduObj.params.data_coding = defs.consts.ENCODING[smsOptions.encoding];
	}
...
function sendLongSms(smsOptions, cb) {
	const	retPduObjs	= [],
		...
		encoding	= smsOptions.encoding || defs.encodings.detect(smsOptions.message), // Set encoding once for all message parts
		smsIds	= [],
		that	= this,
